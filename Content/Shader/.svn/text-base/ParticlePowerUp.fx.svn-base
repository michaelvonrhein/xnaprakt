...
VS_OUTPUT ParticleVSHigh(VS_INPUT In)
{
    ...
    if (In.TextureCoord0.y == 0)
 	{
 		Out.Position += float4(In.Position.xyz * 2, 0);
    ...
    if ((int(In.TextureCoord0.x) % 4) == 0)
    {
		Out.Position.x      += cos(-In.TextureCoord0.x * time) * amplitude * 0.8;
		Out.Position.y		+= sin(-In.TextureCoord0.x * time) * amplitude * 0.8;
		Out.Position.z		+= sin(-In.TextureCoord0.x * time) * amplitude * 0.8;
	}
	else if ((int(In.TextureCoord0.x) % 4) == 1)
	{
		Out.Position.x      += cos(-In.TextureCoord0.x * time) * amplitude * 0.8;
		Out.Position.y		-= sin(-In.TextureCoord0.x * time) * amplitude * 0.8;
		Out.Position.z		+= sin(-In.TextureCoord0.x * time) * amplitude * 0.8;
	}
	else if ((int(In.TextureCoord0.x) % 4) == 2)
	{
		Out.Position.x      += cos(-In.TextureCoord0.x * time) * amplitude;
		Out.Position.y		-= sin(-In.TextureCoord0.x * time) * amplitude;
	}
	else
	{
		Out.Position.x      += cos(-In.TextureCoord0.x * time) * amplitude;
		Out.Position.z		-= sin(-In.TextureCoord0.x * time) * amplitude;
	}
	
    Out.Position.x *= time * speed;
    Out.Position.y *= time * speed;
    Out.Position.z *= time * speed;
...
{
	return tex2D(ColorSamp, In.TextureCoord0) * float4(1, 1, 1, 0.5 - time * 0.5);
}
