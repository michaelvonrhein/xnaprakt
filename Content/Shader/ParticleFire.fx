...
VS_OUTPUT ParticleVSHigh(VS_INPUT In)
{
    ...
    Out.Position.x      += sin(-In.TextureCoord0.x * time / 10);
    Out.Position.y		+= fmod(-In.TextureCoord0.x * time, fmod(In.TextureCoord0.x, 5.0));
    Out.Position.z		+= cos(-In.TextureCoord0.x * time / 6);
...
{
	return tex2D(ColorSamp, In.TextureCoord0) * float4(1, 1, 1, 0.3);
}
